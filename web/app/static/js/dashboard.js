        ...
        async function init() {
            ...
            try {
                ...
                console.log("DEBUG Dashboard: UID =", auth.currentUser.uid);
                console.log("DEBUG Dashboard: DATA =", DATA);
                console.log("DEBUG Dashboard: TXS =", ALL_TRANSACTIONS);

                if (DATA.error) { 
                    console.warn('Backend reported no data, initializing with defaults.');
        ...
        async function generatePortfolioInsight() {
            ...
            try {
                const res = await tokenFetch('/api/portfolio/analysis', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    `).join('');
        ...
        function renderKPIs() {
            const s = DATA.summary;
            // El % se calcula SOLO sobre capital de inversión (sin efectivo)
    ...
    <span class="dleg-pct">${tot > 0 ? (vals[i] / tot * 100).toFixed(1) : 0}%</span></div>`).join('');
        ...
        function renderAlloc() {
            ...
            document.getElementById('alloc-list').innerHTML = Object.entries(g).map(([name, gr], i) => {
                ...
                const profitHTML = isCash ? '' : `<span style="font-size:11px;margin-left:6px;color:${profit >= 0 ? 'var(--green)' : 'var(--red)'};">${profit >= 0 ? '+' : ''}${pp}%</span>`;
                const subHTML = isCash ? `${p.toFixed(1)}% del patrimonio` : `${p.toFixed(1)}% del patrimonio · ${eur(gr.total_inv)} invertido`;
        ...
        function renderTable() {
            ...
            Object.entries(g).forEach(([name, gr]) => {
                ...
                const isCash = name === 'CASH';
                const groupProfitHTML = isCash ? '' : `<span class="gval ${profit >= 0 ? 'up' : 'down'}">${profit >= 0 ? '+' : ''}${eur(profit)}</span>`;
  ...
  `;
        ...
        function renderCategoryPopover(container, txId) {
            ...
            CATEGORIES_CACHE.forEach(cat => {
                ...
                cat.subcategories.forEach(sub => {
                    ...
                    btn.onclick = () => updateTransactionCategory(txId, cat.name, sub.name, cat.id, sub.id);
        ...
        async function updateTransactionCategory(txId, catName, subName, catId, subId) {
            await tokenFetch(`/api/transactions/${txId}`, { 
                method: 'PATCH', 
                headers: { 'Content-Type': 'application/json' }, 
                body: JSON.stringify({ 
                    category: catName, 
        ...
        async function toggleReviewed(txId, status) {
            await tokenFetch(`/api/transactions/${txId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_reviewed: status }) });
            fetchTransactions();
        }
        async function linkTransactions(childId, parentId) {
            await tokenFetch('/api/transactions/link', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ child_id: childId, parent_id: parentId }) });
            fetchTransactions();
        }
        async function unlinkTransaction(txId) {
            await tokenFetch('/api/transactions/unlink', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: txId }) });
            fetchTransactions();
        }
        ...
        async function deleteTransaction(txId) {
            ...
            try { const r = await tokenFetch(`/api/transactions/${txId}`, { method: 'DELETE' }); if (r.ok) { fetchTransactions(); if (typeof loadFinanceData === 'function') loadFinanceData(); } else alert('Error al eliminar'); }
            catch (e) { alert('Error de conexión'); }
        }
    ...
    <div class="sub-row">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">
        ...
        async function renameCategory(id, currentName) {
            ...
            if (!newName || newName === currentName) return;
            await tokenFetch('/api/categories', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: id, name: newName }) });
            fetchCategories();
        }
        async function renameSubcategory(id, currentName) {
            ...
            if (!newName || newName === currentName) return;
            await tokenFetch('/api/subcategories', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: id, name: newName }) });
            fetchCategories();
        }
        async function addCategory() { const n = document.getElementById('new-cat-name').value; if (!n) return; await tokenFetch('/api/categories', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: n }) }); document.getElementById('new-cat-name').value = ''; fetchCategories(); }
        async function deleteCategory(id) { if (!confirm('¿Seguro? Se borrarán sus subcategorías.')) return; await tokenFetch(`/api/categories/${id}`, { method: 'DELETE' }); fetchCategories(); }
        async function addSubcategory(catId) { const inp = document.getElementById(`new-sub-${catId}`); const n = inp.value; if (!n) return; await tokenFetch('/api/subcategories', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: n, category_id: catId }) }); inp.value = ''; fetchCategories(); }
      ...
      <div style="display:flex;gap:8px">
        ...
        function onSearchInput(val) {
            ...
            searchTimeoutGlobal = setTimeout(async () => {
                ...
                const r = await tokenFetch(`/api/search?q=${val}&portfolio=${port}`); const results = await r.json();
        ...
        async function saveAsset() {
            ...
            let holdingsVal = document.getElementById('asset-holdings').value;
            let investedVal = document.getElementById('asset-invested').value;
            
            if (document.getElementById('asset-subtype').value === 'cash') {
                investedVal = holdingsVal; // En efectivo, la rentabilidad es siempre 0
                if (document.getElementById('cash-sync-mode').value === 'auto') {
                    finalType = 'auto';
                    finalTicker = document.getElementById('cash-bank-source').value;
                }
            }
            const data = { id: document.getElementById('asset-id').value || null, name: document.getElementById('asset-name').value, portfolio: document.getElementById('asset-portfolio').value, subtype: document.getElementById('asset-subtype').value, type: finalType, ticker: finalTicker, holdings: holdingsVal, invested_total: investedVal };
            const r = await tokenFetch('/api/configs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
            if (r.ok) { switchTab('list'); fetchConfigs(); }
        }
        function editConfig(id) {
            const c = allConfigs.find(x => x.id === id); if (!c) return;
            
            // Buscar el valor actual calculado en los datos del dashboard
            let currentHoldings = c.holdings;
            if (DATA && DATA.portfolios_grouped) {
                for (const g of Object.values(DATA.portfolios_grouped)) {
                    ...
                    if (found) { 
                        // Para efectivo usamos el valor total de dinero, para el resto la cantidad de holdings
                        currentHoldings = (c.subtype === 'cash') ? found.value : found.holdings; 
                        break; 
                    }
                }
            }
            ...
            setTimeout(() => { 
                ...
                document.getElementById('asset-holdings').value = currentHoldings; // <--- Valor actualizado
        ...
        async function syncIndexa() {
            ...
            try { await tokenFetch('/api/sync/all', { method: 'POST' }); await init(); } catch (e) { }
